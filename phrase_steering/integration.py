"""Runtime integration for original MESM plus phrase Query Steering."""
...
from .dataset import PhraseCharadesDataset, configure_phrase_dataset, phrase_collate
...
def _build_model(args, vocab=None):
    ...
    if args.phrase_steering_layer != 0:
        raise ValueError("This implementation supports phrase_steering_layer=0 only.")
...
def install_phrase_steering() -> None:
    """Patch original runner factories before train.py/eval.py imports."""
    ...
    _INSTALLED = True
    LOGGER.info("Installed original MESM phrase Query Steering integration.")
