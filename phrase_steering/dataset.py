...
def configure_phrase_dataset(opt) -> None:
    ...
    _RUNTIME_CONFIG = PhraseDatasetConfig(
        ...
        require_semantic_sidecar=bool(
            getattr(opt, "require_semantic_sidecar", True)
        ),
    )
...
class PhraseCharadesDataset(CharadesDataset):
    """Charades-STA with deterministic phrase masks and Level-1 candidates."""
    ...
    def __init__(self, ann_path, feat_files, split,
        ...
        if self.enable_competitor_loss and max_gather_size > 0:
            raise ValueError(
                "Level-1 competitor sampling requires max_gather_size=-1 so that "
                "all annotations from a video remain in one dataset item."
            )
    ...
    def _load_semantic_sidecar(self) -> Dict[int, dict]:
        ...
        if sidecar_path is None or not sidecar_path.is_file():
            if semantic_required and self.require_semantic_sidecar:
                expected = sidecar_path or Path("<semantic_sidecar_dir is not set>")
                raise FileNotFoundError(
                    f"Phrase steering is enabled but the {self.split} sidecar is missing: "
                    f"{expected}. Generate the v1.2 sidecar before training/evaluation."
                )
            return {}
        ...
        with sidecar_path.open("r", encoding="utf-8") as handle:
            for line_number, line in enumerate(handle, start=1):
                ...
                if line_id in records:
                    raise ValueError(
                        f"Duplicate line_id={line_id} in {sidecar_path} at JSONL line {line_number}."
                    )
    ...
    def _semantic_fields(self, line_id: int, video_id: str, sentence: str, semantic_by_id):
        if line_id not in semantic_by_id:
            if (self.enable_phrase_steering or self.enable_competitor_loss) and self.require_semantic_sidecar:
                raise KeyError(
        ...
        if _normalize_sentence(semantic.get("sentence", "")) != _normalize_sentence(sentence):
            raise ValueError(
                ...
                "The annotation and sidecar must be regenerated from the same source file."
            )
...
def phrase_collate(batch):
    """Extend the original Charades collate without changing its public fields."""
