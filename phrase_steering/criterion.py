"""Training-only same-video competitor loss for original MESM."""
...
class PhraseCriterion(Criterion):
    ...
    def _zero_competitor(self, outputs):
    ...
    def loss_competitor(self, outputs, targets, indices, log=True):
        ...
        if not required_outputs.issubset(outputs) or "competitor_index" not in targets:
            return self._zero_competitor(outputs)
    ...
    def forward(self, outputs, targets, is_training=True):
        ...
        if "aux_outputs" in outputs:
            for layer_index, aux_outputs in enumerate(outputs["aux_outputs"]):
                auxiliary_indices = self.matcher(aux_outputs, targets)
                for loss_name in self.losses:
                    if loss_name in {"saliency", "rec_ss", "rec_fw", "competitor"}:
                        continue
