"""Phrase encoder, target query steering, and MESM forward integration."""
...
class ActionObjectPhraseEncoder(nn.Module):
    ...
    def forward(
        ...
        effective_valid = (
            phrase_valid
            & action_mask.any(dim=1)
            & object_mask.any(dim=1)
        )
...
class PhraseQuerySteering(nn.Module):
    ...
    def forward(
        ...
        phrase_valid = phrase_valid.to(
            device=video_content.device,
            dtype=torch.bool,
        )
        ...
        coefficient = (video_content * unit_phrase.unsqueeze(1)).sum(
            dim=-1,
            keepdim=True,
        )
...
class PhraseSteeredT2VEncoder(nn.Module):
    """Apply steering once, immediately before the first T2V layer."""
...
class PhraseMESM(MESM):
    """MESM-v2 with phrase composition and positive-path target steering."""
    ...
    def _encode_phrase(self, projected_words, words_mask, kwargs):
    ...
    def forward(self, video_feat, video_mask, words_id, words_mask, words_weight, num_clips, **kwargs):
        ...
        # The existing cross-video negative path intentionally receives no phrase
        # token. Competition is auxiliary-only and never steers a negative query.
