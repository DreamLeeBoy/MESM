#!/usr/bin/env python3
"""CUDA forward/backward smoke test for original MESM Query Steering."""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=(
            "./config/charades/"
            "C+SF_C_original_MESM_phrase_steering_v1_2.json"
        ),
    )
...
def main():
    ...
    competitor_candidates = [
        ...
        if any(
            len(candidate_ids) > 0
            for candidate_ids in meta["level1_candidate_line_ids"]
        )
    ]
    if not competitor_candidates:
        raise RuntimeError("No Level-1 competitor video was found in the training sidecar.")
    ...
    if batch["num_clips"].numel() < 2:
        raise AssertionError("The smoke batch must contain at least two video groups.")
    if int((batch["competitor_index"] >= 0).sum()) == 0:
        raise AssertionError("The selected batch has no valid Level-1 competitor.")
    ...
    if hasattr(model, "query_content_embed"):
        raise AssertionError("v2 query-content embedding was unexpectedly retained.")
    if hasattr(model.transformer, "path_encoder"):
        raise AssertionError("Pathformer was unexpectedly retained.")
    ...
    print("architecture: original MESM + phrase steering")
    print("Pathformer present:", hasattr(model.transformer, "path_encoder"))
